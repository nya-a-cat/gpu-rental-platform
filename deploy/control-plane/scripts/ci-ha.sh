...
phase1_catalog_migration_row="$(run_in_postgres "SELECT version || '|' || checksum FROM control_plane_schema_migrations WHERE version = '000004_phase1_resource_catalog';")"
...
phase1_catalog_migration_checksum="${phase1_catalog_migration_row#*|}"
migration_tables="$(run_in_postgres "SELECT string_agg(table_name, ',' ORDER BY table_name) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ANY (ARRAY['accelerator_profiles','audit_events','audit_events_default','capacity_pools','clusters','gpu_devices','idempotency_records','node_pools','nodes','operations','outbox_events','project_quotas','projects','quota_reservations','resource_classes','resource_inventories','resource_providers','resource_traits','role_bindings','tenants']::text[]);")"
...
[[ "${phase1_catalog_migration_row%%|*}" == "000004_phase1_resource_catalog" ]]
[[ "${phase0_migration_checksum}" =~ ^[0-9a-f]{64}$ ]]
[[ "${phase1_migration_checksum}" =~ ^[0-9a-f]{64}$ ]]
[[ "${phase1_isolation_migration_checksum}" =~ ^[0-9a-f]{64}$ ]]
[[ "${phase1_catalog_migration_checksum}" =~ ^[0-9a-f]{64}$ ]]
[[ "${migration_tables}" == "accelerator_profiles,audit_events,audit_events_default,capacity_pools,clusters,gpu_devices,idempotency_records,node_pools,nodes,operations,outbox_events,project_quotas,projects,quota_reservations,resource_classes,resource_inventories,resource_providers,resource_traits,role_bindings,tenants" ]]
jq -n \
  ...
  --arg phase1CatalogChecksum "${phase1_catalog_migration_checksum}" \
  --arg tables "${migration_tables}" \
  '{status:"passed",migrations:[{version:"000001_phase0_foundation",checksum:$phase0Checksum},{version:"000002_phase1_tenancy",checksum:$phase1Checksum},{version:"000003_phase1_shared_isolation",checksum:$phase1IsolationChecksum},{version:"000004_phase1_resource_catalog",checksum:$phase1CatalogChecksum}],requiredTables:($tables | split(","))}' \
