...
if [[ "${mode}" == "full" ]]; then
  ...
  check_json migration-validation.json '.status == "passed" and (.migrations | map(.version)) == ["000001_phase0_foundation","000002_phase1_tenancy"] and all(.migrations[]; (.checksum | test("^[0-9a-f]{64}$"))) and (.requiredTables | sort) == (["audit_events","audit_events_default","idempotency_records","operations","outbox_events","project_quotas","projects","quota_reservations","role_bindings","tenants"] | sort)' "migration validation failed"
