...
if [[ "${mode}" == "full" ]]; then
  ...
  check_json migration-validation.json '.status == "passed" and .version == "000001" and (.checksum | test("^[0-9a-f]{64}$")) and (.requiredTables | sort) == (["audit_events","audit_events_default","idempotency_records","operations","outbox_events"] | sort)' "migration validation failed"
