...
if [[ "${mode}" == "full" ]]; then
  ...
  check_json migration-validation.json '.status == "passed" and (.migrations | map(.version)) == ["000001_phase0_foundation","000002_phase1_tenancy","000003_phase1_shared_isolation","000004_phase1_resource_catalog"] and all(.migrations[]; (.checksum | test("^[0-9a-f]{64}$"))) and (.requiredTables | sort) == (["accelerator_profiles","audit_events","audit_events_default","capacity_pools","clusters","gpu_devices","idempotency_records","node_pools","nodes","operations","outbox_events","project_quotas","projects","quota_reservations","resource_classes","resource_inventories","resource_providers","resource_traits","role_bindings","tenants"] | sort)' "migration validation failed"
