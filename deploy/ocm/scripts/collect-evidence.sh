...
CSR_FILTER='{apiVersion,kind,items:[.items[]|{metadata:{name:.metadata.name,uid:.metadata.uid,resourceVersion:.metadata.resourceVersion,creationTimestamp:.metadata.creationTimestamp,labels:.metadata.labels},spec:{signerName:.spec.signerName,expirationSeconds:.spec.expirationSeconds,usages:.spec.usages},status:{conditions:[.status.conditions[]?|{type,status,lastTransitionTime,lastUpdateTime}]}}]}'
...
kubectl --context "${HUB_CONTEXT}" get certificatesigningrequests -o json 2>/dev/null | jq "${CSR_FILTER}" >"${ARTIFACT_DIR}/certificate-signing-requests.json" || echo '{"captureStatus":"unavailable"}' >"${ARTIFACT_DIR}/certificate-signing-requests.json"
