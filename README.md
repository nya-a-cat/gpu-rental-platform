...
GPU inventory and instance delivery are simulated. The Docker profile uses the real NestJS API, MongoDB documents, Redis sessions and Redis reservation lock. Delivered SSH, web terminal and notebook addresses use the reserved `.invalid` domain and are intentionally unreachable. The project does not claim physical provisioning, real payment processing or live GPU telemetry.
