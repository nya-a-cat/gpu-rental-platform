...
func TestBuildWorkIsDeterministicAndRequestsWholeGPU(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if string(first.Manifest) != string(second.Manifest) { t.Fatal("ManifestWork output changed between identical inputs") }
	var document map[string]any
	if err := json.Unmarshal(first.Manifest, &document); err != nil { t.Fatal(err) }
	encoded, _ := json.Marshal(document)
	if len(encoded) == 0 || first.WorkID != WorkName(input.Workspace.ID) { t.Fatal("workspace work identity was not deterministic") }
}
...
func TestBuildWorkStopsByRemovingComputeManifests(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	var document map[string]any
	if err := json.Unmarshal(work.Manifest, &document); err != nil { t.Fatal(err) }
	spec := document["spec"].(map[string]any)
	workload := spec["workload"].(map[string]any)
	if manifests := workload["manifests"].([]any); len(manifests) != 0 { t.Fatalf("stopped workspace produced %d compute manifests", len(manifests)) }
}
