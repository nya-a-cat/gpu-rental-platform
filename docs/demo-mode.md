...
Demo mode does not allocate physical GPUs, start containers, accept payments, open reachable SSH or notebook sessions, or report live utilization, temperature, IP addresses or host status. Simulated connection data always uses reserved `.invalid` domains. Resource, order and instance changes exist only in the current browser profile.
