...
当前 `/api/v1/instances` 首个生产切片提供 GPU Workspace 的异步创建、读取和 `desiredState` 更新。创建请求从 AcceleratorProfile 读取整卡 GPU 数量，并由 OCM ManifestWork 生成带 `nvidia.com/gpu` 请求的 StatefulSet 与 headless Service；observed/provisioning 回写协调器、PVC、访问网关和快照在后续 Phase 1 任务中交付。
