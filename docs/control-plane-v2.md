...
本轮 Phase 0 工作聚焦可独立验证的生产基础。完成状态以 GitHub Actions 和 `progress.md` 的测试证据为准。
...
| Fleet      | 固定 OCM 1.3.1 与双 kind 集群脚本，覆盖 ManagedCluster、CSR、证书、Lease 和 ManifestWork |
| Add-on     | 建立 Addon Framework 1.3.0 manager/agent、最小权限、脱敏容量指纹和 OCM Lease             |
| 可观测性   | 暴露存活、就绪和 Prometheus 指标入口；保留 Trace Context 头契约，OTel 链路后续接入       |
| 部署验证   | 独立 v2 Compose 与 OCM conformance 均由 Actions 执行；硬件 GPU 验证进入自托管门禁        |

ManagedCluster 与 Add-on 的代码和验证入口已经进入仓库，当前认证状态等待 GitHub Actions 证据。当前库存 Generation 是 Phase 0 聚合容量指纹；Placement、Reservation 与 Allocation 将使用带 Agent Epoch、序列和 Fencing 数据的单调 ResourceProvider generation。真实 GPU 调度、财务计费和生产租户隔离尚未完成。固定版本与证据边界见 [Kubernetes 1.34 首个认证矩阵](certification/kubernetes-1.34-matrix.md)。
...
OCM 负责 ManagedCluster 双向确认、证书轮换、Cluster UID、Lease、ManifestWork、Placement 和 Add-on 生命周期。GPU Platform Add-on 额外维护 Agent Epoch、命令序列号、Fencing Token、库存、Allocation、UsageFact 与访问隧道，并支持控制面 N/N-1 兼容。

集群状态分别记录 `Connected`、`Schedulable`、`InventoryFresh` 和 `ExecutionHealthy`。产品目标采用默认心跳 15 秒、异常 45 秒、离线 90 秒，并允许部署配置调整。Phase 0 Agent 当前每 15 秒上报库存，OCM Framework Lease 使用其固定续期周期；45/90 秒判定与独立 InventoryFresh Condition 仍属于后续控制面任务。
...
计费事实分成四层：不可变 `UsageFact`、带价格版本的 `RatedUsage`、不可变 `LedgerEntry` 和账期 `Invoice`。迟到或乱序事实可重放，价格调整不改写历史，账本调整使用冲正。内置 BillingEngine 先支持按秒固定单价、预算和基础账单；OpenMeter 在 Phase 2 进行两个完整账期的影子双算。
