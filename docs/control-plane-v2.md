...
计费事实分成四层：不可变 `UsageFact`、带价格版本的 `RatedUsage`、不可变 `LedgerEntry` 和账期 `Invoice`。迟到或乱序事实可重放，价格调整不改写历史，账本调整使用冲正。控制面已提供按资源单位秒计价、版本化价格簿、货币和向上取整规则的内置 BillingEngine，并通过 `/api/v1/usage-facts` 保存不可变原始事实和 RatedUsage 结果；账本流水随后接入同一接口。OpenMeter 在 Phase 2 进行两个完整账期的影子双算。
