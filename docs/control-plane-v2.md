...
| 部署验证   | 独立 v2 Compose 栈由 Actions 启动并冒烟验证；OCM Hub 与真实 GPU Add-on 后续接入    |
...
集群状态分别记录 `Connected`、`Schedulable`、`InventoryFresh` 和 `ExecutionHealthy`。默认心跳 15 秒、异常 45 秒、离线 90 秒，可由部署配置调整。
